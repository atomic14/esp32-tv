...
AudioOutput::AudioOutput(i2s_port_t i2s_port) : m_i2s_port(i2s_port)
{
...
void AudioOutput::write(SampleT *samples, int count, int shift)
{
  int sample_index = 0;
...
void AudioOutput::write(int8_t *samples, int count)
{
  write(samples, count, 8);
}

void AudioOutput::write(int16_t *samples, int count)
{
  write(samples, count, 0);
}
